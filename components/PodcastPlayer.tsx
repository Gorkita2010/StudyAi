...
const PodcastPlayer: React.FC<PodcastPlayerProps> = ({ studyMaterial, language, systemLanguage, theme }) => {
    ...
    const handleGenerate = async () => {
        setLoading(true);
        setAudioSrc(null);
        const buffer = await generatePodcast(studyMaterial, language, coverage);
    ...
    const handleDownloadDevice = () => {
        ...
        const byteArray = new Uint8Array(byteNumbers);
        const blob = new Blob([byteArray], { type: 'audio/wav' });
        downloadFile(blob, `StudyAi_Podcast_${Date.now()}.wav`, 'audio/wav');
    }
    ...
    if (!audioSrc && !loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[300px] space-y-6 animate-fadeIn text-center max-w-md mx-auto">
                <div className="text-6xl">🎧</div>
                <p className={theme.textSecondary}>{t('podcastDesc', systemLanguage)}</p>
                
                <div className="w-full">
                    <label className={`block text-sm font-bold mb-2 ${theme.textMain}`}>{t('coverageLabel', systemLanguage)}</label>
                    <select 
                        value={coverage}
                        onChange={(e) => setCoverage(parseInt(e.target.value))}
                        className={`w-full px-4 py-2 rounded-lg outline-none border-2 ${theme.inputBg}`}
                    >
                        <option value="100">{t('coverage100', systemLanguage)}</option>
                        <option value="90">{t('coverage90', systemLanguage)}</option>
                        <option value="80">{t('coverage80', systemLanguage)}</option>
                        <option value="70">{t('coverage70', systemLanguage)}</option>
                        <option value="60">{t('coverage60', systemLanguage)}</option>
                        <option value="50">{t('coverage50', systemLanguage)}</option>
                    </select>
                </div>
        ...
        <div className="flex flex-col items-center justify-center w-full max-w-lg mx-auto animate-fadeIn space-y-6">
            ...
            <div className={`relative w-full h-48 rounded-2xl overflow-hidden bg-black border border-slate-700 shadow-2xl flex items-center justify-center`}>
                ...
                <div className="absolute bottom-2 right-4 text-xs font-mono text-slate-400 bg-black/50 px-2 py-1 rounded">
                    {formatTime(currentTime)} / {getRemainingTime()}
                </div>
            </div>
            ...
            <div className="w-full flex items-center space-x-3">
                ...
                />
                <span className="text-xs font-mono text-slate-400">{getRemainingTime()}</span>
            </div>
            ...
            <div className="flex items-center justify-between w-full gap-4">
                 ...
                 <button 
                    onClick={togglePlay}
                    className={`px-8 py-3 rounded-full font-bold text-xl shadow-lg transition-transform hover:scale-105 ${theme.primaryBtn}`}
                 >
                 ...
                 <div className="flex flex-col gap-1">
                    <button 
                        ...
                        className={`px-4 py-1.5 rounded-lg font-bold text-xs flex items-center justify-center ${theme.secondaryBtn}`}
                    >
                        ...
                        {t('downloadDevice', systemLanguage)}
                    </button>

                    <button 
                        onClick={handleSaveToDrive}
                        disabled={saving}
                        className={`px-4 py-1.5 rounded-lg font-bold border text-xs transition-colors flex items-center justify-center ${theme.cardBorder} ${theme.textSecondary} hover:bg-slate-800`}
                    >
                        <svg className="w-3 h-3 mr-1" viewBox="0 0 24 24"><path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                        {saving ? t('saving', systemLanguage) : "Drive"}
                    </button>
                 </div>
            </div>
        </div>
