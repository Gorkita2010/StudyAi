...
const StudyHub: React.FC<StudyHubProps> = ({ config, onExit, systemLanguage, theme }) => {
  ...
  // Density State
  const [density, setDensity] = useState<'dense' | 'medium' | 'concise'>('medium');

  const loadContent = async (type: 'summary' | 'outline' | 'mindmap') => {
      setLoading(true);
      const result = await generateStudyContent(config.studyMaterial, type, config.language, density);
  ...
  const handleDownload = (type: 'summary' | 'outline' | 'mindmap') => {
      let content = "";
      let ext = "txt";
      let mime = "text/plain";
      ...
      if (type === 'summary') { content = summary; ext="md"; }
      if (type === 'outline') { content = outline; ext="md"; }
      // Mindmap download is handled inside the component usually, but here we can download source
      if (type === 'mindmap') { content = mindMap; ext="mmd"; }
  ...
  useEffect(() => {
      if (activeTab === 'summary' && !loading) loadContent('summary');
  ...
  // Reload content when density changes if we are on summary tab
  useEffect(() => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 flex flex-col h-[90vh]">
          ...
          <div className="flex items-center space-x-4">
             ...
             {activeTab === 'summary' && (
                 <select 
                    ...
                    className={`px-3 py-1 rounded border text-sm ${theme.inputBg}`}
                 >
                     <option value="dense">{t('densityDense', systemLanguage)}</option>
                     <option value="medium">{t('densityMedium', systemLanguage)}</option>
                     <option value="concise">{t('densityConcise', systemLanguage)}</option>
                 </select>
             )}
          </div>
          <button onClick={onExit} className={`${theme.secondaryBtn} px-4 py-2 rounded-lg text-sm font-bold`}>
      ...
      <div className={`flex-grow overflow-hidden rounded-xl shadow-2xl border flex flex-col ${theme.cardBg} ${theme.cardBorder}`}>
          <div className="flex-grow overflow-y-auto p-8 scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-transparent relative">
            {renderContent()}
          </div>
          
          {['summary', 'outline', 'mindmap'].includes(activeTab as string) && (
            <div className={`p-4 border-t ${theme.cardBorder} flex justify-end space-x-4 bg-opacity-50 bg-black/20 backdrop-blur-md`}>
                
                <button
                    onClick={() => handleDownload(activeTab as any)}
                    disabled={loading || !summary && !outline && !mindMap}
                    className={`px-6 py-2 rounded-lg font-bold text-sm flex items-center transition-colors ${theme.secondaryBtn}`}
                >
                ...
                <button
                    ...
                    disabled={saving || loading || !summary && !outline && !mindMap}
                    className={`px-6 py-2 rounded-lg font-bold text-sm flex items-center border transition-colors ${theme.cardBorder} ${theme.textSecondary} hover:bg-slate-500/10`}
                >
