...
const Auth: React.FC<AuthProps> = ({ onLogin, systemLanguage, theme }) => {
    ...
    const handleGuest = () => {
        // Create a guest session locally
        const guestUser: User = {
            id: 'guest', // Static ID allows caching on this device
            name: t('guestName', systemLanguage),
            email: '',
            avatar: '👤'
        };
        // Persist guest in local DB so previous guest sessions can be restored
        // (Using the same AuthService logic as manual users)
        try {
            // We just treat it as a login to ensure session starts
            onLogin(guestUser);
        } catch(e) {
            console.error(e);
        }
            ...
            <div className={`p-8 rounded-2xl shadow-2xl ${theme.cardBg} ${theme.cardBorder}`}>
                ...
                <div className="space-y-3 mt-6">
                    <button 
                        ...
                        disabled={loading}
                        className={`w-full py-3 rounded-xl font-bold border flex items-center justify-center transition-colors hover:bg-slate-500/10 bg-white text-slate-700 ${theme.cardBorder}`}
                    >
                        {loading ? (
                    ...
                    <button 
                        ...
                        onClick={handleGuest}
                        disabled={loading}
                        className={`w-full py-3 rounded-xl font-bold border border-dashed flex items-center justify-center transition-colors ${theme.cardBorder} ${theme.textSecondary} hover:${theme.textMain} hover:bg-slate-500/10`}
                    >
                        <svg className="w-5 h-5 mr-2 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                        {t('guestBtn', systemLanguage)}
                    </button>
                </div>
